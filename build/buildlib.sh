...
function installPKG {
    if [[ "$#" -eq 0 ]]; then log 2 "Requires: [PKG(s) to install]"; return 3
        ...
        if [[ ! "$EUID" -eq 0 ]]; then log -1 "Installing ${PKG[*]}"
...
function findProcess {
    if hasCMD pgrep &>/dev/null; then
        if [[ "$#" -eq 1 ]]; then declare -r PROCESS="$1"
            if pgrep "$PROCESS" &>/dev/null; then return 0
            else return 1
            fi
...
function findFullProcess {
    if hasCMD pgrep &>/dev/null; then
        if [[ "$#" -eq 1 ]]; then local -r PROCESS="$1"
            if pgrep --full "$PROCESS" &>/dev/null; then return 0
            else return 1
            fi
        else log 2 "Requires argument: process"; return 2
        fi
...
function launch_install_qemu {
    ...
    IMGOUT="${IMG}-$( date +%s )"
    if ! cp --reflink=auto -v "$IMG" "$IMGOUT"; then log 2 "Copying IMG failed"; return 1
    fi
    if hasCMD pgrep; then
        if findProcess qemu-system-aarch64; then log 2 "QEMU is already running"; return 1
        fi
...
function launch_qemu {
    ...
    if ! isDirectory qemu-raspbian-network; then git clone https://github.com/nachoparker/qemu-raspbian-network.git
    fi
...
function launch_installation {
    ...
    elif isZero "$INSTALLATION_STEPS"; then log 2 "No installation instructions provided"; return 3
    fi
    local PREINST_CODE="
set -e$DBG
sudo su
...
"
    ...
    if ! ssh_pi "$IP" "$PREINST_CODE" "$INSTALLATION_CODE" "$INSTALLATION_STEPS"; then
        log 2 "SSH installation failed to QEMU target at: $IP"; return 4
    fi
}
...
function launch_installation_qemu
{
    ...
    if notMatch "$NO_CFG_STEP" "$MATCH"; then CFG_STEP=configure
    fi
    if notMatch "$NO_CLEANUP" "$MATCH"; then CLEANUP_STEP="if [[ \$( type -t cleanup ) == function ]];then cleanup; fi"
    fi
    if notMatch "$NO_HALT_STEP" "$MATCH"; then HALT_STEP="nohup halt &>/dev/null &"
    fi
...
function prepare_dirs {
    ...
    if notMatch "$CLEAN" "0"; then rm --recursive --force "${DIRS[2]}"
    fi
...
function mount_raspbian {
    ...
    log -1 "Mounting: $MP"
    if ! hasCMD fdisk; then installPKG fdisk
    fi
    ...
    if isRoot; then
        if ! mount "$IMG" -o offset="$OFFSET" "$MP"; then log 2 "Failed to mount IMG at: $MP"; return 4
        fi
    else if ! sudo mount "$IMG" -o offset="$OFFSET" "$MP"; then log 2 "Failed to mount IMG at: $MP"; return 4
         fi
    fi
...
function mount_raspbian_boot {
    ...
    log -1 "Mountpoint: $MP"
    mkdir -p "$MP"
    if isRoot; then if ! mount "$IMG" -o offset="$OFFSET" "$MP"; then log 2 "Failed to mount IMG at: $MP"; return 4
    fi
    else if ! sudo mount "$IMG" -o offset="$OFFSET" "$MP"; then log 2 "Failed to mount IMG at: $MP"; return 4
    fi
    fi
...
function umount_raspbian {
    ...
    if isDirectory "$ROOTDIR"; then
        if isRoot; then
            if ! umount --lazy "$ROOTDIR"; then log 2 "Could not unmount: $ROOTDIR"; return 1; fi
            if ! rmdir "$ROOTDIR"; then log 2 "Could not remove: $ROOTDIR"; return 2; fi
             ...
             if ! sudo rmdir "$ROOTDIR"; then log 2 "Could not remove: $ROOTDIR"; return 2; fi
        fi
    fi
    if isDirectory "$BOOTDIR"; then
        if isRoot; then
            if ! umount --lazy "$BOOTDIR"; then log 2 "Could not unmount: $BOOTDIR"; return 3; fi
            if ! rmdir "$BOOTDIR"; then log 2 "Could not remove: $BOOTDIR"; return 4; fi
             ...
             if ! sudo rmdir "$BOOTDIR"; then log 2 "Could not remove: $BOOTDIR"; return 4; fi
        fi
    fi
...
function prepare_sshd_raspbian {
    ...
    else if ! sudo touch "$BOOTDIR"/ssh; then log 2 "Failed to create SSH file in IMG boot"; return 3; fi
    fi; umount_raspbian
}
...
function download_raspbian {
    ...
    else if ! wget "$URL" -nv -O "$ZIP_CACHE"; then log 2 "Download failed from: $URL"; return 3; fi
    fi
