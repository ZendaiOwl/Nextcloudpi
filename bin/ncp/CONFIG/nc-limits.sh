...
tmpl_php_max_memory() {
  local TOTAL_MEM="$( get_total_mem )"
  local MEMORYLIMIT="$(findAppParameter nc-limits MEMORYLIMIT)"
  [[ "$MEMORYLIMIT" == "0" ]] && echo -n "$(( TOTAL_MEM * 75 / 100 ))" || echo -n "$MEMORYLIMIT"
}

tmpl_php_max_filesize() {
  local FILESIZE="$(findAppParameter nc-limits MAXFILESIZE)"
  [[ "$FILESIZE" == "0" ]] && echo -n "10G" || echo -n "$FILESIZE"
}

tmpl_php_threads() {
  local PHPTHREADS="$(findAppParameter nc-limits PHPTHREADS)"
...
configure()
{
  ...
  local CONF=/etc/php/${PHP_VERSION}/fpm/conf.d/90-ncp.ini
  ...
  # MAX PHP THREADS
  local CONF=/etc/php/${PHP_VERSION}/fpm/pool.d/www.conf
  ...
  [[ "$require_fpm_restart" == "true" ]] && {
    bash -c "sleep 3; source /usr/local/etc/library.sh; clearOPCache; service php${PHP_VERSION}-fpm restart" &>/dev/null &
  }
